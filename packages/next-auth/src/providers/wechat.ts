...
export default function WeChat<P extends WeChatProfile>(
  ...
  const authorizationEndpointUrlCustom = process.env.NEXT_PUBLIC_URL_WECHAT_AUTH ?? authorizationEndpointUrl
  const authorization: AuthorizationEndpointHandler = {
    url: authorizationEndpointUrlCustom,
    params: {
      appid: clientId,
      response_type: "code",
      scope: authorizationScope,
      state: Math.random().toString(),
      redirect_uri: `${process.env.NEXT_PUBLIC_BASE_URL}/api/auth/callback/wechat`,
    },
  }
  ...
  const token: TokenEndpointHandler = {
    ...
    async request({ params, provider }) {
        ...
        const urlWithParams = `${tokenEndpointUrl}?${urlparams.toString()}`;
        const res = await fetch(urlWithParams).then((res) => res.json())
  ...
  const profile = async (profile: WeChatProfile) => {
    return {
      id: profile.unionid,
